---
title: 'Explanatory Coexistence: R Code for Wrangling PsychoPy Data Wrangling UK'
author: "Rui Wang"
date: "`r Sys.Date()`"
...
files = list.files(path = "UKDataRaw", pattern="*.csv", full.names = TRUE)

# Combine all raw data files
combinefiles <- data.frame()
for (i in 1:length(files)) {
  data_read <- read_csv(files[i], guess_max = 10000)
  if (nrow(combinefiles) == 0) {
    combinefiles <- data_read
  } else {
    common_cols <- intersect(names(combinefiles), names(data_read))
    if (length(common_cols) > 0) {
      data_read <- data_read[, common_cols, drop = FALSE]
      combinefiles <- rbind(combinefiles[, common_cols, drop = FALSE], data_read)
    }
  }
}
combinefiles <- as_tibble(combinefiles)
colnames(combinefiles)[10] = "ParticipantID"
...
ALLdata <- combinefiles %>%  mutate (ParticipantID = as.factor(ParticipantID)) %>% 
  mutate (expName = as.factor(expName)) %>% 
  mutate (OS = as.factor(OS))
write.csv(ALLdata, "ALL_data_UK.csv", row.names = FALSE, sep="\t")
```
...
write.csv(ALLstopsignal, "ALL_StopSignal_UK.csv", row.names = FALSE, sep="\t")
```
...
#Save the R data table that was created here as a TXT file. TXT was selected becuase it is readable in a variety of programms (excel, SPSS, etc.) and becuase CSV files created problems for the procedures here. The TXT file is tab delimited - making it easy to import. 
write.csv(TRIALSstopsignal, "TRIALS_StopSignal_UK.csv", row.names = FALSE, sep="\t")
```
...
fmdata1 <- dplyr::rename (.data = ALLdata, FM_instruction_image = FMInstrImage) %>% 
  ...
  mutate(FM_trials_blocktype = recode_factor (FM_block, 
                         `CCCC` = "pure", 
                         `SSSS` = "pure",
                         `SSCC` = "mixed",
                         `CCSS` = "mixed")) %>% 
...
#Save the R data table that was created here as a TXT file. TXT was selected becuase it is readable in a variety of programms (excel, SPSS, etc.) and becuase CSV files created problems for the procedures here. The TXT file is tab delimited - making it easy to import. 
write.csv(ALLfigurematching, "ALL_FigureMatching_UK.csv", row.names = FALSE, sep="\t")
```
...
#Save the R data table that was created here as a TXT file. TXT was selected becuase it is readable in a variety of programms (excel, SPSS, etc.) and becuase CSV files created problems for the procedures here. The TXT file is tab delimited - making it easy to import. 
write.csv(TRIALSfigurematching, "TRIALS_FigureMatching_UK.csv", row.names = FALSE, sep="\t")
```
```{r Flanker: Tidy}
...
summary(ALLflanker)
#Save the R data table that was created here as a TXT file. TXT was selected becuase it is readable in a variety of programms (excel, SPSS, etc.) and becuase CSV files created problems for the procedures here. The TXT file is tab delimited - making it easy to import. 
write.csv(ALLflanker, "ALL_Flanker_UK.csv", row.names = FALSE, sep="\t")
```
...
#Save the R data table that was created here as a TXT file. TXT was selected becuase it is readable in a variety of programms (excel, SPSS, etc.) and becuase CSV files created problems for the procedures here. The TXT file is tab delimited - making it easy to import. 
write.csv(TRIALSflanker, "TRIALS_Flanker_UK.csv", row.names = FALSE, sep="\t")
```
```{r Animism: Tidy}
...
summary(ALLanimism)
#Save the R data table that was created here as a TXT file. TXT was selected becuase it is readable in a variety of programms (excel, SPSS, etc.) and becuase CSV files created problems for the procedures here. The TXT file is tab delimited - making it easy to import. 
write.csv(ALLanimism, "ALL_Animism_UK.csv", row.names = FALSE, sep="\t")
```
...
#Save the R data table that was created here as a TXT file. TXT was selected becuase it is readable in a variety of programms (excel, SPSS, etc.) and becuase CSV files created problems for the procedures here. The TXT file is tab delimited - making it easy to import. 
write.csv(TRIALSanimism, "TRIALS_Animism_UK.csv", row.names = FALSE, sep="\t")
```
...
#Save the R data table that was created here as a TXT file. TXT was selected becuase it is readable in a variety of programms (excel, SPSS, etc.) and becuase CSV files created problems for the procedures here. The TXT file is tab delimited - making it easy to import. 
write.csv(ALLstatements, "ALL_ScienceStatements_UK.csv", row.names = FALSE, sep="\t")
```
...
# Create a summary of the data so that it can be checked here and used to correct any other issues
TRIALSstatements <- TRIALSstatements[TRIALSstatements$RS_trials_image != "Plants turn food into energy.", ]
write.csv(TRIALSstatements, "TRIALS_ScienceStatements_UK.csv", row.names = FALSE) 
...
btdata <- dplyr::rename (.data = ALLdata, BT_instruction_key_resp.keys = pt_instr_key_resp.keys) %>% 
  ...
  # Create new columns to code for accuracy and determine RT to correct trials
  mutate(BT_trials_accuracy = recode_factor (BT_trials_accuracyscore, 
                         `1` = "correct", 
                         `0` = "incorrect")) %>%
  mutate(BT_trials_consistency = recode_factor (BT_trials_intuition, 
                         `Neutral` = "NA", 
...
summary(ALLBuoyancy)
#Save the R data table that was created here as a TXT file. TXT was selected becuase it is readable in a variety of programms (excel, SPSS, etc.) and becuase CSV files created problems for the procedures here. The TXT file is tab delimited - making it easy to import. 
write.csv(ALLBuoyancy, "ALL_BuoyancyTask_UK.csv", row.names = FALSE, sep="\t")
```
...
#Save the R data table that was created here as a TXT file. TXT was selected becuase it is readable in a variety of programms (excel, SPSS, etc.) and becuase CSV files created problems for the procedures here. The TXT file is tab delimited - making it easy to import. 
write.csv(TRIALSBuoyancy, "TRIALS_BuoyancyTask_UK.csv", row.names = FALSE, sep="\t")
```
...
MRIBdata <- dplyr::rename (.data = ALLdata, MRIB_instruction_key_resp.keys = MRIB_instr_key_resp.keys) %>% 
  ...
  mutate (MRIB_difficulty_level = recode_factor (MRIB_ss_level, 
                         `_ss1_` = "level_1", 
                         `_ss2_` = "level_2",
                         `_ss3_` = "level_3",
                         `ss1_m` = "level_1",
                         `ss2_m` = "level_2",
                         `ss3_m` = "level_3"))%>%
  mutate (MRIB_md_level = recode_factor (MRIB_md_level, 
                         `_pd` = "pd", 
                         `_md` = "md",
                         `md.` = "md"))%>%
  ...
  # Create new columns to code for accuracy and determine RT to correct trials
  mutate(MRIB_trials_accuracy = recode_factor (MRIB_trials_accuracyscore, 
                         `1` = "correct", 
                         `0` = "incorrect")) %>%
...
summary(ALLmrib)
#Save the R data table that was created here as a TXT file. TXT was selected becuase it is readable in a variety of programms (excel, SPSS, etc.) and becuase CSV files created problems for the procedures here. The TXT file is tab delimited - making it easy to import. 
write.csv(ALLmrib, "ALL_mribdata_UK.csv", row.names = FALSE, sep="\t")
```
```{r MatrixReasoning: Create file for analyses}
...
summary(TRIALSmrib)
#Save the R data table that was created here as a TXT file. TXT was selected becuase it is readable in a variety of programms (excel, SPSS, etc.) and becuase CSV files created problems for the procedures here. The TXT file is tab delimited - making it easy to import. 
write.csv(TRIALSmrib, "TRIALS_MRIB_UK.csv", row.names = FALSE, sep="\t")
# Delete any blank rows (basically these will be the ones that are not part of the task or are the instruction, break, end of task rows)
```
```{r CorsiBlock: Tidy}
CorsiBlockdata<-read.csv("ALL_CorsiBlock_UK.csv")
...
TRIALScorsi <- dplyr::select(.data = cbdata, 
                        ParticipantID, CB_trials_show.keys,CB_trials_response.keys, CB_trials_response.rt, CB_trials_perclick_response.rt, CB_trials_correctclick, CB_trials_totalclick, CB_trials_version, CB_trials_correctanswer, CB_trials_accuracyscore,CB_trials_accuracy, CB_difficultylevel, CB_difficultylevel_number,CB_trials_RTcorrect,CB_trials_RTcorrect_perclick)
# Create a summary of the data so that it can be checked here and used to correct any other issues
summary(TRIALScorsi)
#Save the R data table that was created here as a TXT file. TXT was selected becuase it is readable in a variety of programms (excel, SPSS, etc.) and becuase CSV files created problems for the procedures here. The TXT file is tab delimited - making it easy to import.
write.csv(TRIALScorsi, "TRIALS_CorsiBlock_UK.csv", row.names = FALSE, sep="\t")
```
`